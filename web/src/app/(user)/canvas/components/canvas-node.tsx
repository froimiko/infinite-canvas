...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    const panelContent = showPanel && renderPanel ? renderPanel(data) : null;
    ...
    return (
        ...
        >
            ...
            {panelContent ? <div className="absolute left-1/2 top-full z-[70] w-[500px] -translate-x-1/2 pt-4">{panelContent}</div> : null}
        </div>
    );
});

function NodeContent(props: NodeContentRendererProps) {
    if ((props.node.type === CanvasNodeType.Config || props.node.type === CanvasNodeType.NovelAI) && props.renderNodeContent) return props.renderNodeContent(props.node);
...
const nodeContentRenderers = {
    ...
    [CanvasNodeType.NovelAI]: EmptyImageContent,
} satisfies Record<CanvasNodeType, (props: NodeContentRendererProps) => ReactNode>;

function LoadingContent({ node, theme }: Pick<NodeContentRendererProps, "node" | "theme">) {
    // queueHint 只有 NovelAI SSE 路径会写（免费生图串行排队），其余情况保持原样显示「生成中」。
    const hint = node?.metadata?.queueHint;
    return (
        <div className="flex h-full w-full flex-col items-center justify-center gap-3 px-4 text-center" style={{ color: theme.node.activeStroke }}>
            <div className="size-10 animate-spin rounded-full border-2" style={{ borderColor: theme.node.stroke, borderTopColor: theme.node.activeStroke }} />
            <span className={hint ? "text-[10px] leading-4" : "text-[10px] tracking-[0.2em]"}>{hint || "生成中"}</span>
        </div>
    );
}
