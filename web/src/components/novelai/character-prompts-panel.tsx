...
export function CharacterPromptsPanel({ value, useAutoPositioning, onChange, theme }: CharacterPromptsPanelProps) {
    ...
    return (
        <div className="space-y-2.5">
            ...
            {!value.length ? (
                <div className="rounded-xl border border-dashed px-3 py-4 text-center text-xs" style={{ borderColor: theme.node.stroke, color: theme.node.muted }}>
                    开启角色分离后可添加最多 6 个角色
                </div>
            ) : null}
                ...
                <div key={index} className="space-y-2 rounded-xl border p-2.5" style={{ borderColor: theme.node.stroke, background: theme.node.fill }}>
                    <div className="flex items-center gap-2">
                        <input
                            ...
                            maxLength={20}
                            onChange={(event) => updateCharacter(index, "displayName", event.target.value)}
                            className="min-w-0 flex-1 rounded-lg border bg-transparent px-2 py-1 text-xs outline-none"
                    ...
                    <PromptBlockEditor
                        ...
                        onChange={(nextValue) => updateCharacter(index, "characterPrompt", nextValue)}
                        tokens={character.characterPromptTokens?.length ? character.characterPromptTokens : undefined}
                        onTokensChange={(nextTokens) => updateCharacter(index, "characterPromptTokens", nextTokens)}
                        compact
                        rows={2}
                        className="rounded-lg text-xs"
                        style={{ background: theme.node.fill, borderColor: theme.node.stroke, color: theme.node.text }}
                        placeholder="角色外观、服装、姿态..."
                    />
                    <PromptBlockEditor
                        value={character.characterNegativePrompt || ""}
                        onChange={(nextValue) => updateCharacter(index, "characterNegativePrompt", nextValue)}
                        tokens={character.characterNegativePromptTokens?.length ? character.characterNegativePromptTokens : undefined}
                        onTokensChange={(nextTokens) => updateCharacter(index, "characterNegativePromptTokens", nextTokens)}
                        compact
                        rows={1}
                        className="rounded-lg text-xs"
                        style={{ background: theme.node.fill, borderColor: theme.node.stroke, color: theme.node.text }}
                        placeholder="角色负面提示词（可选）"
                    />
                    {!useAutoPositioning ? (
                        <div className="flex items-center gap-2 text-xs" style={{ color: theme.node.muted }}>
                            ...
                            <label className="flex items-center gap-1">
                                ...
                                <input
                                    ...
                                    min={0}
                                    max={4}
                                    value={character.coords?.x ?? 2}
                                    onChange={(event) => updateCoord(index, "x", Number(event.target.value))}
                                    className="w-12 rounded border bg-transparent px-1 py-0.5 text-center outline-none"
                                    style={{ borderColor: theme.node.stroke, color: theme.node.text }}
                                />
                            </label>
                            <label className="flex items-center gap-1">
                                ...
                                <input
                                    ...
                                    min={0}
                                    max={4}
                                    value={character.coords?.y ?? 2}
                                    onChange={(event) => updateCoord(index, "y", Number(event.target.value))}
                                    className="w-12 rounded border bg-transparent px-1 py-0.5 text-center outline-none"
                                    style={{ borderColor: theme.node.stroke, color: theme.node.text }}
                                />
                            </label>
