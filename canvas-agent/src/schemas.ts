...
const generationOptionsSchema = z.object({
  ...
  novelAIEnabled: z.boolean().optional(),
  novelAIModel: z.string().optional(),
  novelAISampler: z.string().optional(),
  novelAISteps: z.number().optional(),
  novelAICfgScale: z.number().optional(),
  novelAISeed: z.number().optional(),
  novelAIUcPreset: z.string().optional(),
  novelAICfgRescale: z.number().optional(),
  novelAINoiseSchedule: z.string().optional(),
  novelAISm: z.boolean().optional(),
  novelAISmDyn: z.boolean().optional(),
  novelAIDynamicThresholding: z.boolean().optional(),
  novelAIVarietyPlus: z.boolean().optional(),
  novelAIAqtPreset: z.string().optional(),
  novelAIDivideRoles: z.boolean().optional(),
  novelAIUseAutoPositioning: z.boolean().optional(),
  novelAICharacterPrompts: z.array(z.record(z.unknown())).optional(),
});
...
export const toolInputSchemas = {
  ...
  canvas_connect_nodes: z.object({
    connections: z
      .array(z.object({ fromNodeId: z.string(), toNodeId: z.string() }))
      .min(1),
  }),
...
export const toolDescriptions: Record<ToolName, string> = {
  ...
  canvas_create_config_node:
    "创建生成配置节点，可指定 text/image/video/audio 模式和生成参数，可选择立即触发生成。",
  canvas_create_image_prompt_flow:
    "创建提示词文本节点和图片生成配置节点，并自动连线，可选择立即触发生图。",
  canvas_create_generation_flow:
    "创建通用生成流程：提示词文本节点、生成配置节点、参考节点连线，可用于文案、生图、视频或音频。",
  ...
  canvas_run_generation:
    "触发指定节点生成，通常用于配置节点或文本/图片/视频/音频节点。",
};
