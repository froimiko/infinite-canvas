...
export class CanvasSession {
  ...
  async callTool(name: unknown, rawInput: unknown) {
    ...
    if (tool === "canvas_create_text_node") {
      ...
      input = {
        ops: [
          textNodeOp(
            text,
            text.x ?? nextCanvasX(this.canvasState),
            text.y ?? 0
          ),
        ],
      };
    ...
    if (tool === "canvas_create_image_prompt_flow") {
      input = {
        ops: generationFlowOps(
          { ...(input as Record<string, unknown>), mode: "image" },
          this.canvasState
        ),
      };
      tool = "canvas_apply_ops";
    ...
    if (tool === "canvas_create_config_node") {
      ...
      input = {
        ops: [
          configNodeOp(configId, data, x, y),
          ...(data.autoRun ? [runGenerationOp(configId, mode, prompt)] : []),
        ],
      };
      tool = "canvas_apply_ops";
    }
    ...
    if (
      tool === "canvas_generate_text" ||
      tool === "canvas_generate_image" ||
      tool === "canvas_generate_video" ||
      tool === "canvas_generate_audio"
    ) {
      input = {
        ops: generationFlowOps(
          {
        ...
        ops: [
          {
            ...
            metadata:
              data.freeResize === undefined
                ? undefined
                : { freeResize: data.freeResize },
          },
        ],
      };
      tool = "canvas_apply_ops";
    ...
    if (tool === "canvas_run_generation") {
      ...
      input = {
        ops: [
          runGenerationOp(data.nodeId, generationMode(data.mode), data.prompt),
        ],
      };
      tool = "canvas_apply_ops";
    }
  ...
  private async requestCanvasTool(
    name: ToolName,
    input: Record<string, unknown>
  ) {
    const requestId = crypto.randomUUID();
    const client =
      this.clients.get(this.canvasState?.clientId || "") ||
...
function configNodeOp(
  ...
  return {
    ...
    metadata: cleanRecord({
      ...
      novelAIEnabled: input.novelAIEnabled,
      novelAIModel: input.novelAIModel,
      novelAISampler: input.novelAISampler,
      novelAISteps: input.novelAISteps,
      novelAICfgScale: input.novelAICfgScale,
      novelAISeed: input.novelAISeed,
      novelAIUcPreset: input.novelAIUcPreset,
      novelAICfgRescale: input.novelAICfgRescale,
      novelAINoiseSchedule: input.novelAINoiseSchedule,
      novelAISm: input.novelAISm,
      novelAISmDyn: input.novelAISmDyn,
      novelAIDynamicThresholding: input.novelAIDynamicThresholding,
      novelAIVarietyPlus: input.novelAIVarietyPlus,
      novelAIAqtPreset: input.novelAIAqtPreset,
      novelAIDivideRoles: input.novelAIDivideRoles,
      novelAIUseAutoPositioning: input.novelAIUseAutoPositioning,
      novelAICharacterPrompts: input.novelAICharacterPrompts,
    }),
  };
}
...
function generationFlowOps(
  ...
  const referenceNodeIds = Array.isArray(input.referenceNodeIds)
    ? input.referenceNodeIds.filter(
        (id): id is string => typeof id === "string"
      )
    : [];
  const tokens = [
    `@[node:${textId}]`,
    ...referenceNodeIds.map((id) => `@[node:${id}]`),
  ];
  const configInput = { ...input, prompt: tokens.join("\n") };
  return [
    textNodeOp(
      { id: textId, text: prompt, title: String(input.title || "提示词") },
      x,
      y
    ),
    ...
    ...(input.autoRun
      ? [runGenerationOp(configId, mode, tokens.join("\n"))]
      : []),
  ];
}
...
function generationMode(value: unknown): "text" | "image" | "video" | "audio" {
  return value === "text" || value === "video" || value === "audio"
    ? value
    : "image";
}
