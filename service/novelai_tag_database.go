...
func PromptTagTranslationDatabaseStatus() (model.PromptTagTranslationDatabaseStatus, error) {
	...
	if err != nil { return model.PromptTagTranslationDatabaseStatus{}, err }
...
func PromptTagTranslationDatabaseAssets() ([]model.PromptTagTranslationAsset, error) {
	...
	for _, item := range release.Assets {
		if !strings.HasSuffix(strings.ToLower(item.Name), ".csv") { continue }
		asset := model.PromptTagTranslationAsset{Name: item.Name, Size: item.Size, DownloadURL: item.BrowserDownloadURL, ReleaseTag: release.TagName}
		if saved, ok := installed[asset.Name]; ok { asset.Installed = strings.TrimSpace(saved.Error) == ""; asset.InstalledAt = saved.InstalledAt; asset.Error = saved.Error }
...
func TranslatePromptTags(tags []string) (map[string]string, error) {
	setting, err := promptTagDatabaseSettingForQuery(); if err != nil { return nil, err }
	if setting.Enabled != nil && !*setting.Enabled { return map[string]string{}, nil }
	translationSetting, err := promptTagTranslationDatabaseSettingForQuery(); if err != nil { return nil, err }
	return repository.TranslatePromptTags(tags, translationSetting.Enabled)
}
...
func importPromptTagTranslationCSV(setting model.PromptTagTranslationDatabaseSetting, asset model.PromptTagTranslationAsset, content []byte) error {
	reader := csv.NewReader(bytes.NewReader(content)); reader.FieldsPerRecord = -1
	header, err := reader.Read(); if err != nil { return safeMessageError{message: "第三方翻译 CSV 表头异常"} }
	columns := promptTagTranslationCSVColumns(header)
	for _, name := range []string{"name", "category", "cn_name", "post_count"} { if _, ok := columns[name]; !ok { return safeMessageError{message: "第三方翻译 CSV 缺少字段：" + name} } }
	updatedAt := now(); batch := []model.PromptTagExternalTranslation{}
	flush := func() error { if len(batch) == 0 { return nil }; err := repository.UpsertPromptTagExternalTranslations(batch); batch = []model.PromptTagExternalTranslation{}; return err }
	for {
		record, err := reader.Read(); if err == io.EOF { break }; if err != nil { return err }
		item := promptTagExternalTranslationFromRecord(record, columns, setting, asset, updatedAt)
		if item.Name == "" || item.CNName == "" { continue }
		batch = append(batch, item); if len(batch) >= 1000 { if err := flush(); err != nil { return err } }
	}
	return flush()
}
...
func applyPromptTagGitHubHeaders(request *http.Request, wantsJSON bool) { request.Header.Set("User-Agent", promptTagGitHubUserAgent); if wantsJSON { request.Header.Set("Accept", "application/json") }; if token := promptTagGitHubToken(); token != "" { request.Header.Set("Authorization", "Bearer "+token) } }
func promptTagGitHubToken() string { if token := strings.TrimSpace(os.Getenv("PROMPT_TAG_GITHUB_TOKEN")); token != "" { return token }; return strings.TrimSpace(os.Getenv("GITHUB_TOKEN")) }
func promptTagGitHubErrorMessage(prefix string, response *http.Response, body []byte) string { message := fmt.Sprintf("%s：GitHub 返回 %d", prefix, response.StatusCode); if response.StatusCode == http.StatusForbidden { message += "，可能是 GitHub 匿名 API 限流或云端出口被风控；可配置 PROMPT_TAG_GITHUB_TOKEN 或 GITHUB_TOKEN 后重试" }; if reset := strings.TrimSpace(response.Header.Get("X-RateLimit-Reset")); reset != "" { message += "，RateLimit-Reset=" + reset }; if len(body) > 0 { text := strings.TrimSpace(string(body)); if len(text) > 180 { text = text[:180] + "..." }; if text != "" { message += "，响应：" + text } }; return message }
func normalizePromptTagTreePath(value string) (string, error) { value = path.Clean(strings.TrimLeft(strings.TrimSpace(value), "/")); if value == "." { value = "" }; if value != "tags" && value != "danbooru" && !strings.HasPrefix(value, "tags/") && !strings.HasPrefix(value, "danbooru/") { return "", safeMessageError{message: "仅允许浏览 WeiLin tags/danbooru 目录"} }; if strings.Contains(value, "..") { return "", safeMessageError{message: "数据库路径不合法"} }; return value, nil }
func normalizePromptTagSQLPath(value string, packageType model.PromptTagPackageType) (string, error) { value = path.Clean(strings.TrimLeft(strings.TrimSpace(value), "/")); if value == "." || strings.Contains(value, "..") || !strings.HasSuffix(strings.ToLower(value), ".sql") { return "", safeMessageError{message: "仅允许安装 WeiLin SQL 文件"} }; if packageType == model.PromptTagPackageTypeTags && !strings.HasPrefix(value, "tags/") { return "", safeMessageError{message: "tags 类型只能安装 tags/ 目录下的 SQL"} }; if packageType == model.PromptTagPackageTypeDanbooru && !strings.HasPrefix(value, "danbooru/") { return "", safeMessageError{message: "danbooru 类型只能安装 danbooru/ 目录下的 SQL"} }; if !strings.HasPrefix(value, "tags/") && !strings.HasPrefix(value, "danbooru/") { return "", safeMessageError{message: "仅允许安装 WeiLin tags/danbooru SQL"} }; return value, nil }
func escapePromptTagPath(value string) string { parts := strings.Split(strings.TrimLeft(value, "/"), "/"); for i := range parts { parts[i] = url.PathEscape(parts[i]) }; return strings.Join(parts, "/") }
func promptTagInstalledPackage(path string) (bool, model.PromptTagInstalledPackage) { packages, err := repository.ListPromptTagInstalledPackages(); if err != nil { return false, model.PromptTagInstalledPackage{} }; for _, item := range packages { if item.Path == path { return true, item } }; return false, model.PromptTagInstalledPackage{} }
func promptTagTranslationInstalledPackageMap() map[string]model.PromptTagTranslationInstalledPackage { items, err := repository.ListPromptTagTranslationInstalledPackages(); if err != nil { return map[string]model.PromptTagTranslationInstalledPackage{} }; result := make(map[string]model.PromptTagTranslationInstalledPackage, len(items)); for _, item := range items { result[item.AssetName] = item }; return result }
func promptTagInstalledPackageRecord(setting model.PromptTagDatabaseSetting, packageType model.PromptTagPackageType, packagePath string, sha string, size int64, errorMessage string) model.PromptTagInstalledPackage { nowValue := now(); if packageType == "" { packageType = promptTagPackageTypeFromPath(packagePath) }; return model.PromptTagInstalledPackage{Path: strings.TrimSpace(packagePath), Type: packageType, SourceOwner: setting.Owner, SourceRepo: setting.Repo, Branch: setting.Branch, SHA: sha, Size: size, InstalledAt: nowValue, UpdatedAt: nowValue, Error: strings.TrimSpace(errorMessage)} }
func promptTagTranslationInstalledPackageRecord(setting model.PromptTagTranslationDatabaseSetting, asset model.PromptTagTranslationAsset, size int64, errorMessage string) model.PromptTagTranslationInstalledPackage { nowValue := now(); return model.PromptTagTranslationInstalledPackage{AssetName: asset.Name, SourceOwner: setting.Owner, SourceRepo: setting.Repo, ReleaseTag: asset.ReleaseTag, Size: size, InstalledAt: nowValue, UpdatedAt: nowValue, Error: strings.TrimSpace(errorMessage)} }
func PromptTagInstallHasFailure(result PromptTagInstallResult) bool { return len(result.Failed) > 0 }
func promptTagInstallError(result PromptTagInstallResult) error { if !PromptTagInstallHasFailure(result) { return nil }; messages := make([]string, 0, len(result.Failed)); for _, item := range result.Failed { if strings.TrimSpace(item.Error) != "" { messages = append(messages, item.Path+": "+item.Error) } }; if len(messages) == 0 { return errors.New("部分 WeiLin SQL 安装失败") }; return errors.New(strings.Join(messages, "; ")) }
